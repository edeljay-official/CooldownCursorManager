...
addonTable.ApplyUnitFrameCustomization = function()
  ...
  if ufEnabled and profile.ufBigHBEnabled == true then
  ...
  local function ApplyBorderTextureColor(frameObj, r, g, b)
    ...
    if frameObj == PlayerFrame then
      local altTex = frameObj and frameObj.PlayerFrameContainer and frameObj.PlayerFrameContainer.AlternatePowerFrameTexture
      if altTex and altTex.SetVertexColor then
        altTex:SetVertexColor(r or 1, g or 1, b or 1, 1)
      end
    end
  end
  ...
  local function ApplyUFBigHBOverlayHealthColor(o, hp, unit)
    ...
    local r, g, b
    if profile.useBiggerPlayerHealthframeClassColor and unit and UnitExists(unit) and UnitIsPlayer(unit) then
  ...
  local function ApplyUFBigHBForFrame(key, rootFrame, hp, mp, portrait, isPlayerFrame)
    local enabled = ufEnabled and profile.ufBigHBEnabled == true
    local frameTex
    if key == "player" then
    ...
    if not enabled or not rootFrame or not hp or not hp.GetWidth then
      ...
      if (key == "target" or key == "focus") and rootFrame and rootFrame.ReputationColor and existing and existing.repColorSaved then
        if existing.repColorShown and rootFrame.ReputationColor.Show then rootFrame.ReputationColor:Show() end
      end
      if (key == "target" or key == "focus") and rootFrame and existing and existing.highLevelTexSaved then
        local contentFrame = rootFrame.GetParent and rootFrame:GetParent()
      ...
      if key == "target" and rootFrame and existing then
        if existing.targetNameSaved and rootFrame.Name then
          if existing.targetNameShown and rootFrame.Name.Show then rootFrame.Name:Show() elseif rootFrame.Name.Hide then rootFrame.Name:Hide() end
          if rootFrame.Name.SetTextColor and existing.targetNameColorR then
            rootFrame.Name:SetTextColor(existing.targetNameColorR, existing.targetNameColorG, existing.targetNameColorB)
          end
          if existing.targetNamePoint and rootFrame.Name.ClearAllPoints and rootFrame.Name.SetPoint then
            rootFrame.Name:ClearAllPoints()
            rootFrame.Name:SetPoint(existing.targetNamePoint, existing.targetNameRelTo, existing.targetNameRelPt, existing.targetNameOrigX or 0, existing.targetNameOrigY or 0)
          end
        end
        if existing.targetLevelSaved and rootFrame.LevelText then
          if existing.targetLevelShown and rootFrame.LevelText.Show then rootFrame.LevelText:Show() elseif rootFrame.LevelText.Hide then rootFrame.LevelText:Hide() end
          if rootFrame.LevelText.SetTextColor and existing.targetLevelColorR then
            rootFrame.LevelText:SetTextColor(existing.targetLevelColorR, existing.targetLevelColorG, existing.targetLevelColorB)
          end
          if existing.targetLevelPoint and rootFrame.LevelText.ClearAllPoints and rootFrame.LevelText.SetPoint then
            rootFrame.LevelText:ClearAllPoints()
            rootFrame.LevelText:SetPoint(existing.targetLevelPoint, existing.targetLevelRelTo, existing.targetLevelRelPt, existing.targetLevelOrigX or 0, existing.targetLevelOrigY or 0)
          end
        end
      end
      if key == "focus" and rootFrame and existing then
        if existing.nameColorSaved and rootFrame.Name then
          if existing.nameOrigShown and rootFrame.Name.Show then rootFrame.Name:Show() elseif rootFrame.Name.Hide then rootFrame.Name:Hide() end
          if rootFrame.Name.SetTextColor and existing.nameOrigColorR then
            rootFrame.Name:SetTextColor(existing.nameOrigColorR, existing.nameOrigColorG, existing.nameOrigColorB)
          end
          if existing.namePoint and rootFrame.Name.ClearAllPoints and rootFrame.Name.SetPoint then
            rootFrame.Name:ClearAllPoints()
            rootFrame.Name:SetPoint(existing.namePoint, existing.nameRelTo, existing.nameRelPt, existing.nameOrigX or 0, existing.nameOrigY or 0)
          end
        end
        if existing.levelColorSaved and rootFrame.LevelText then
          if existing.levelOrigShown and rootFrame.LevelText.Show then rootFrame.LevelText:Show() elseif rootFrame.LevelText.Hide then rootFrame.LevelText:Hide() end
          if rootFrame.LevelText.SetTextColor and existing.levelOrigColorR then
            rootFrame.LevelText:SetTextColor(existing.levelOrigColorR, existing.levelOrigColorG, existing.levelOrigColorB)
          end
          if existing.levelPoint and rootFrame.LevelText.ClearAllPoints and rootFrame.LevelText.SetPoint then
            rootFrame.LevelText:ClearAllPoints()
            rootFrame.LevelText:SetPoint(existing.levelPoint, existing.levelRelTo, existing.levelRelPt, existing.levelOrigX or 0, existing.levelOrigY or 0)
          end
        end
      end
    ...
    o.bgTex:SetVertexColor(borderR, borderG, borderB, 1)
    ...
    if not o.valueHooked and hp.SetValue then
      ...
      hooksecurefunc(hp, "SetValue", function(self, value)
        ...
        if ov and ov.healthFrame and ov.healthFrame.SetValue then
          pcall(ov.healthFrame.SetValue, ov.healthFrame, value)
        end
      end)
      if hp.SetMinMaxValues then
        hooksecurefunc(hp, "SetMinMaxValues", function(self, minVal, maxVal)
          local ov = State.ufBigHBOverlays[hookKey]
          if ov and ov.healthFrame and ov.healthFrame.SetMinMaxValues then
            pcall(ov.healthFrame.SetMinMaxValues, ov.healthFrame, minVal, maxVal)
          end
        end)
      end
      if hp.SetStatusBarColor then
        hooksecurefunc(hp, "SetStatusBarColor", function(self, r, g, b, a)
          ...
          if p and p.useBiggerPlayerHealthframeClassColor and hookKey and UnitExists(hookKey) and UnitIsPlayer(hookKey) then
    ...
    if (key == "target" or key == "focus") and rootFrame and rootFrame.ReputationColor then
      if not o.repColorSaved then
        o.repColorSaved = true
        o.repColorShown = rootFrame.ReputationColor.IsShown and rootFrame.ReputationColor:IsShown() or nil
      end
      if rootFrame.ReputationColor.Hide then rootFrame.ReputationColor:Hide() end
      if not o.repColorHooked and type(hooksecurefunc) == "function" then
        o.repColorHooked = true
        local repKey = key
        hooksecurefunc(rootFrame.ReputationColor, "Show", function(self)
    ...
    if (key == "target" or key == "focus") and rootFrame then
      local contentFrame = rootFrame.GetParent and rootFrame:GetParent()
      local ctxFrame = contentFrame and contentFrame.TargetFrameContentContextual
      local highLvlTex = ctxFrame and ctxFrame.HighLevelTexture
      if highLvlTex then
    ...
    if key == "target" and rootFrame then
      local nameEl = rootFrame.Name
      if nameEl then
        ...
        if profile.ufBigHBHideTargetName then
          ...
          if nameEl.SetTextColor then
            nameEl:SetTextColor(profile.ufBigHBTargetNameColorR or 1, profile.ufBigHBTargetNameColorG or 1, profile.ufBigHBTargetNameColorB or 1)
          end
        end
        local nx = profile.ufBigHBTargetNameX or 0
        local ny = profile.ufBigHBTargetNameY or 0
        if nameEl.ClearAllPoints and nameEl.SetPoint and o.targetNamePoint then
          nameEl:ClearAllPoints()
      ...
      local levelEl = rootFrame.LevelText
      if levelEl then
        ...
        if profile.ufBigHBHideTargetLevel then
          ...
          if levelEl.SetTextColor then
            levelEl:SetTextColor(profile.ufBigHBTargetLevelColorR or 1, profile.ufBigHBTargetLevelColorG or 1, profile.ufBigHBTargetLevelColorB or 1)
          end
        end
        local lx = profile.ufBigHBTargetLevelX or 0
        local ly = profile.ufBigHBTargetLevelY or 0
    ...
    if key == "focus" and rootFrame then
      local nameEl = rootFrame.Name
      if nameEl then
        ...
        local nx = profile.ufBigHBFocusNameX or 0
        local ny = profile.ufBigHBFocusNameY or 0
      ...
      if levelEl then
        ...
        local lx = profile.ufBigHBFocusLevelX or 0
        local ly = profile.ufBigHBFocusLevelY or 0
    ...
    if key == "player" then
      ...
      if nameEl then
        ...
        local nx = profile.ufBigHBPlayerNameX or 0
        local ny = profile.ufBigHBPlayerNameY or 0
        if nameEl.ClearAllPoints and nameEl.SetPoint and o.namePoint then
          nameEl:ClearAllPoints()
          nameEl:SetPoint(o.namePoint, o.nameRelTo, o.nameRelPt, (o.nameOrigX or 0) + nx, (o.nameOrigY or 0) + ny)
        end
      ...
      if levelEl then
        ...
        local lx = profile.ufBigHBPlayerLevelX or 0
        local ly = profile.ufBigHBPlayerLevelY or 0
        if levelEl.ClearAllPoints and levelEl.SetPoint and o.levelPoint then
          levelEl:ClearAllPoints()
          levelEl:SetPoint(o.levelPoint, o.levelRelTo, o.levelRelPt, (o.levelOrigX or 0) + lx, (o.levelOrigY or 0) + ly)
        end
  ...
  do
    ...
    if C_Timer and C_Timer.After and profile.ufBigHBEnabled and profile.ufBigHBPlayerEnabled then
      C_Timer.After(0.5, function()
        ...
        local hp = pHC and pHC.HealthBar
        if not hp then return end
        SyncUFBigHBOverlayValue(ov, hp)
        ApplyUFBigHBOverlayHealthColor(ov, hp, "player")
      end)
    end
  end
  ...
  local function ApplyPlayerHealthColor()
    ...
    local useClassColor = ufEnabled and profile.useBiggerPlayerHealthframeClassColor == true
    if ufEnabled and profile.useBiggerPlayerHealthframe then
  ...
  if not orig.unitHealthColorHooked and type(hooksecurefunc) == "function" and type(UnitFrameHealthBar_Update) == "function" then
    ...
    hooksecurefunc("UnitFrameHealthBar_Update", function(statusBar, unitToken)
      ...
      if ufBigEnabled and State.ufBigHBOverlays then
        local function RefreshBigHBFill(key, root, hpBar)
          ...
          ApplyUFBigHBOverlayHealthColor(ov, hpBar, key)
          if (key == "target" or key == "focus") and root and root.ReputationColor and root.ReputationColor.Hide then
            root.ReputationColor:Hide()
          end
        end
        RefreshBigHBFill("player", pMain, healthBar)
        RefreshBigHBFill("target", tMain, targetBar)
        RefreshBigHBFill("focus", fMain, focusBar)
      end
      ...
      if ufBigEnabled and State.ufBigHBOverlays then
        local function RefreshUFBigOverlayStrata(key)
          local ov = State.ufBigHBOverlays[key]
          ...
          if frameTex and key == "player" then
            ...
            local altPowerTex = PlayerFrame and PlayerFrame.PlayerFrameContainer and PlayerFrame.PlayerFrameContainer.AlternatePowerFrameTexture
            if altPowerTex then
              if altPowerTex.SetFrameStrata then altPowerTex:SetFrameStrata("BACKGROUND") end
              if altPowerTex.SetFrameLevel then altPowerTex:SetFrameLevel(0) end
              if altPowerTex.SetDrawLayer then altPowerTex:SetDrawLayer("BACKGROUND", -8) end
              if ov and ov.altPowerTexHost and ov.altPowerTexParent then
                if ov.altPowerTexHost.SetFrameStrata then ov.altPowerTexHost:SetFrameStrata("BACKGROUND") end
                if ov.altPowerTexHost.SetFrameLevel then ov.altPowerTexHost:SetFrameLevel(0) end
                if ov.altPowerTexHost.SetParent then ov.altPowerTexHost:SetParent(ov.altPowerTexParent) end
                if ov.altPowerTexHost.ClearAllPoints and ov.altPowerTexHost.SetPoint then
                  ov.altPowerTexHost:ClearAllPoints()
                  ov.altPowerTexHost:SetPoint("TOPLEFT", ov.altPowerTexParent, "TOPLEFT", 0, 0)
                  ov.altPowerTexHost:SetPoint("BOTTOMRIGHT", ov.altPowerTexParent, "BOTTOMRIGHT", 0, 0)
                end
                if altPowerTex.SetParent and altPowerTex.GetParent and altPowerTex:GetParent() ~= ov.altPowerTexHost then
                  altPowerTex:SetParent(ov.altPowerTexHost)
                end
                if altPowerTex.ClearAllPoints and altPowerTex.SetPoint then
                  altPowerTex:ClearAllPoints()
                  local restored = false
                  if ov.altPowerTexPoints and #ov.altPowerTexPoints > 0 then
                    for i = 1, #ov.altPowerTexPoints do
                      local pt = ov.altPowerTexPoints[i]
                      if pt and type(pt[1]) == "string" then
                        pcall(altPowerTex.SetPoint, altPowerTex, pt[1], pt[2], pt[3], pt[4], pt[5])
                        restored = true
                      end
                  ...
                  if (not restored) and altPowerTex.SetAllPoints then
                    altPowerTex:SetAllPoints(ov.altPowerTexHost)
                  end
                end
                if ov.altPowerTexShown then
                  if ov.altPowerTexHost.Show then ov.altPowerTexHost:Show() end
                else
                  if ov.altPowerTexHost.Hide then ov.altPowerTexHost:Hide() end
                end
              end
              if ov.altPowerTexShown then
                if altPowerTex.Show then altPowerTex:Show() end
        ...
        RefreshUFBigOverlayStrata("player")
        RefreshUFBigOverlayStrata("target")
        RefreshUFBigOverlayStrata("focus")
      end
    end)
  end
  ...
  local function CollectPlayerGlowTargets()
    ...
    local function addUnitFrameGlows(unitFrame)
      ...
      local tHContainer = tMain and tMain.HealthBarsContainer
      ...
      local tHB = tHContainer and tHContainer.HealthBar
      if tHB then
        add(tHB.OverAbsorbGlow)
        add(tHB.OverHealAbsorbGlow)
        add(tHB.HealAbsorbBar)
        add(tHB.TotalAbsorbBar)
        add(tHB.MyHealPredictionBar)
        add(tHB.OtherHealPredictionBar)
        add(tHB.AnimatedLossBar)
      end
  ...
  if ufEnabled and profile.useBiggerPlayerHealthframe == true then
...
local function GetProfile()
  ...
  if profile.ufBigHBHidePlayerName == nil then profile.ufBigHBHidePlayerName = defaults.profiles.Default.ufBigHBHidePlayerName end
  if profile.ufBigHBHidePlayerLevel == nil then profile.ufBigHBHidePlayerLevel = defaults.profiles.Default.ufBigHBHidePlayerLevel end
  if profile.ufBigHBPlayerNameX == nil then profile.ufBigHBPlayerNameX = defaults.profiles.Default.ufBigHBPlayerNameX end
  if profile.ufBigHBPlayerNameY == nil then profile.ufBigHBPlayerNameY = defaults.profiles.Default.ufBigHBPlayerNameY end
  if profile.ufBigHBPlayerLevelX == nil then profile.ufBigHBPlayerLevelX = defaults.profiles.Default.ufBigHBPlayerLevelX end
  if profile.ufBigHBPlayerLevelY == nil then profile.ufBigHBPlayerLevelY = defaults.profiles.Default.ufBigHBPlayerLevelY end
  if profile.ufBigHBTargetNameX == nil then profile.ufBigHBTargetNameX = defaults.profiles.Default.ufBigHBTargetNameX end
  if profile.ufBigHBTargetNameY == nil then profile.ufBigHBTargetNameY = defaults.profiles.Default.ufBigHBTargetNameY end
  if profile.ufBigHBTargetLevelX == nil then profile.ufBigHBTargetLevelX = defaults.profiles.Default.ufBigHBTargetLevelX end
  if profile.ufBigHBTargetLevelY == nil then profile.ufBigHBTargetLevelY = defaults.profiles.Default.ufBigHBTargetLevelY end
  if profile.ufBigHBHideFocusName == nil then profile.ufBigHBHideFocusName = defaults.profiles.Default.ufBigHBHideFocusName end
  if profile.ufBigHBHideFocusLevel == nil then profile.ufBigHBHideFocusLevel = defaults.profiles.Default.ufBigHBHideFocusLevel end
  if profile.ufBigHBFocusNameX == nil then profile.ufBigHBFocusNameX = defaults.profiles.Default.ufBigHBFocusNameX end
  if profile.ufBigHBFocusNameY == nil then profile.ufBigHBFocusNameY = defaults.profiles.Default.ufBigHBFocusNameY end
  if profile.ufBigHBFocusLevelX == nil then profile.ufBigHBFocusLevelX = defaults.profiles.Default.ufBigHBFocusLevelX end
  if profile.ufBigHBFocusLevelY == nil then profile.ufBigHBFocusLevelY = defaults.profiles.Default.ufBigHBFocusLevelY end
