  ...
  if cooldownFrame then
    ...
    for _, region in ipairs(regions) do
      if region and region.GetObjectType and region:GetObjectType() == "FontString" and region.IsShown and region:IsShown() then
        local okMatch, digits = pcall(function()
          local txt = region:GetText()
          return string.match(tostring(txt or ""), "(%d+)")
        end)
        if okMatch then
...
local defaults = {
  profiles = {
    Default = {
      ...
      useBiggerPlayerHealthframe = false,
      useBiggerPlayerHealthframeClassColor = false,
      useBiggerPlayerHealthframeTexture = "solid",
      ufCustomBorderColorR = 1,
      ufCustomBorderColorG = 1,
      ufCustomBorderColorB = 1,
      ufHealthTextMode = "BOTH",
      ufPowerTextMode = "BOTH",
      ufBigHBEnabled = false,
      ufBigHBPlayerEnabled = false,
      ufBigHBTargetEnabled = false,
      ufBigHBFocusEnabled = false,
      ufBigHBBossEnabled = false,
      ufBigHBPlayerHealthX = 0,
      ufBigHBPlayerHealthY = 0,
      ufBigHBPlayerHealthW = 0,
      ufBigHBPlayerHealthH = 0,
      ufBigHBPlayerBgX = 0,
      ufBigHBPlayerBgY = 0,
      ufBigHBPlayerBgW = 0,
      ufBigHBPlayerBgH = 0,
      ufBigHBPlayerBgBrightness = 1,
      ufBigHBOtherHealthX = 0,
      ufBigHBOtherHealthY = 0,
      ufBigHBOtherHealthW = 0,
      ufBigHBOtherHealthH = 0,
      ufBigHBOtherBgX = 0,
      ufBigHBOtherBgY = 0,
      ufBigHBOtherBgW = 0,
      ufBigHBOtherBgH = 0,
      ufBigHBOtherBgBrightness = 1,
      useBiggerPlayerHealthframeDisableGlows = false,
      useBiggerPlayerHealthframeDisableCombatText = false,
      cdFont = "default",
...
addonTable.ApplyUnitFrameCustomization = function()
  ...
  if profile.ufBigHBEnabled == true then
    profile.useBiggerPlayerHealthframe = false
    profile.useBiggerPlayerHealthframeClassColor = false
  end
  local globalFont, globalOutline = GetGlobalFont()
  local resolvedOutline = (globalOutline ~= "" and globalOutline) or "OUTLINE"
  local function ApplyFontStringGlobal(fs)
    if not fs or not fs.SetFont then return end
    local _, size = fs.GetFont and fs:GetFont()
    fs:SetFont(globalFont, size or 12, resolvedOutline)
  end
  local function ApplyStatusBarTextFonts(bar)
    if not bar then return end
    ApplyFontStringGlobal(bar.LeftText)
    ApplyFontStringGlobal(bar.RightText)
    ApplyFontStringGlobal(bar.TextString)
  end
  local function GetPlayerObjects()
    ...
    return {
      ...
      healthTextLeft = hContainer and hContainer.LeftText,
      healthTextMid = hContainer and hContainer.HealthBarText,
      healthTextRight = hContainer and hContainer.RightText,
    }
  end
  ...
  local function ApplyUnitFrameClassColorsOnly()
    ...
    if not prof or not prof.useBiggerPlayerHealthframeClassColor then return end
  ...
  ApplyFontStringGlobal(player.healthTextLeft)
  ApplyFontStringGlobal(player.healthTextMid)
  ApplyFontStringGlobal(player.healthTextRight)
  ApplyStatusBarTextFonts(healthBar)
  ApplyStatusBarTextFonts(powerBar)
  if PlayerFrameManaBarText then
    ApplyFontStringGlobal(PlayerFrameManaBarText)
  end
  do
    local tMain = TargetFrame and TargetFrame.TargetFrameContent and TargetFrame.TargetFrameContent.TargetFrameContentMain
    local fMain = FocusFrame and FocusFrame.TargetFrameContent and FocusFrame.TargetFrameContent.TargetFrameContentMain
    local tHC = tMain and tMain.HealthBarsContainer
    local fHC = fMain and fMain.HealthBarsContainer
    ApplyFontStringGlobal(tHC and tHC.LeftText)
    ApplyFontStringGlobal(tHC and tHC.HealthBarText)
    ApplyFontStringGlobal(tHC and tHC.RightText)
    ApplyFontStringGlobal(fHC and fHC.LeftText)
    ApplyFontStringGlobal(fHC and fHC.HealthBarText)
    ApplyFontStringGlobal(fHC and fHC.RightText)
    ApplyStatusBarTextFonts(tHC and tHC.HealthBar)
    ApplyStatusBarTextFonts(tMain and tMain.ManaBar)
    ApplyStatusBarTextFonts(fHC and fHC.HealthBar)
    ApplyStatusBarTextFonts(fMain and fMain.ManaBar)
  end
  local function NormalizeUFTextMode(mode)
    local m = type(mode) == "string" and string.upper(mode) or "BOTH"
    if m ~= "NUMERIC" and m ~= "PERCENT" and m ~= "BOTH" and m ~= "NONE" then
      m = "BOTH"
    end
    return m
  end
  local function SafeNumber(v)
    if issecurevariable and issecretvalue and issecretvalue(v) then return nil end
    if type(v) ~= "number" then v = tonumber(v) end
    return v
  end
  local function SetSafeText(fs, txt)
    if fs and fs.SetText then fs:SetText(txt or "") end
  end
  local function SetSafeShown(fs, shown)
  ...
  local function SetAllTexts(left, mid, right, leftText, midText, rightText)
    SetSafeText(left, leftText)
    SetSafeText(mid, midText)
    SetSafeText(right, rightText)
  end
  local ApplyUFTextMode
  local function ApplyUFTextModeToBar(bar, left, mid, right, mode, cur, max)
    local primaryMid = mid or (bar and bar.TextString) or nil
    ApplyUFTextMode(left, primaryMid, right, mode, cur, max)
    if bar and bar.TextString and bar.TextString ~= primaryMid then
      if mode == "NONE" then
        SetSafeText(bar.TextString, "")
      elseif mode == "NUMERIC" then
        local c = SafeNumber(cur)
        if c then
          SetSafeText(bar.TextString, (type(BreakUpLargeNumbers) == "function" and BreakUpLargeNumbers(c)) or tostring(math.floor(c + 0.5)))
        else
          SetSafeText(bar.TextString, "")
        end
      elseif mode == "PERCENT" then
        local c, m = SafeNumber(cur), SafeNumber(max)
        if c and m and m > 0 then
          SetSafeText(bar.TextString, tostring(math.floor((c / m) * 100 + 0.5)) .. "%")
        else
          SetSafeText(bar.TextString, "")
        end
      else
        local c, m = SafeNumber(cur), SafeNumber(max)
        if c and m and m > 0 then
          local pctText = tostring(math.floor((c / m) * 100 + 0.5)) .. "%"
          local numText = (type(BreakUpLargeNumbers) == "function" and BreakUpLargeNumbers(c)) or tostring(math.floor(c + 0.5))
          SetSafeText(bar.TextString, pctText .. "  " .. numText)
        else
          SetSafeText(bar.TextString, "")
        end
      end
  ...
  ApplyUFTextMode = function(left, mid, right, mode, cur, max)
    local c = SafeNumber(cur)
    local m = SafeNumber(max)
    mode = NormalizeUFTextMode(mode)
    if not c or not m or m <= 0 or mode == "NONE" then
      SetAllTexts(left, mid, right, "", "", "")
      return
    end
    local pct = math.floor((c / m) * 100 + 0.5)
    local pctText = tostring(pct) .. "%"
    local numText = (type(BreakUpLargeNumbers) == "function" and BreakUpLargeNumbers(c)) or tostring(math.floor(c + 0.5))
    if mode == "PERCENT" then
      SetAllTexts(left, mid, right, pctText, pctText, "")
      return
    end
    if mode == "NUMERIC" then
      SetAllTexts(left, mid, right, "", numText, numText)
      return
    end
    SetAllTexts(left, mid, right, pctText, pctText, numText)
  end
  local function EnsureUFCustomTextTriplet(key, bar)
    if not bar or not bar.CreateFontString then return nil end
    State.ufCustomTextTriplets = State.ufCustomTextTriplets or {}
    local t = State.ufCustomTextTriplets[key]
    if not t then
      t = {}
      t.left = bar:CreateFontString(nil, "OVERLAY", "GameFontNormalSmall")
      t.mid = bar:CreateFontString(nil, "OVERLAY", "GameFontNormalSmall")
      t.right = bar:CreateFontString(nil, "OVERLAY", "GameFontNormalSmall")
      t.left:SetPoint("LEFT", bar, "LEFT", 2, 0)
      t.mid:SetPoint("CENTER", bar, "CENTER", 0, 0)
      t.right:SetPoint("RIGHT", bar, "RIGHT", -2, 0)
      t.left:SetJustifyH("LEFT")
      t.mid:SetJustifyH("CENTER")
      t.right:SetJustifyH("RIGHT")
      State.ufCustomTextTriplets[key] = t
    end
    ApplyFontStringGlobal(t.left)
    ApplyFontStringGlobal(t.mid)
    ApplyFontStringGlobal(t.right)
    return t
  end
  local function SetDefaultStatusTextsVisible(visible)
    local function ApplyBarTextSuppression(bar, suppress)
      if not bar then return end
      bar.forceHideText = suppress and true or false
      bar.lockShow = suppress and 1 or 0
    ...
    local pHC = pMain and pMain.HealthBarsContainer
    local pPower = PlayerFrameManaBar or (pMain and pMain.ManaBarArea and pMain.ManaBarArea.ManaBar)
    ApplyBarTextSuppression(pHC and pHC.HealthBar, not visible)
    ApplyBarTextSuppression(pPower, not visible)
    SetSafeShown(pHC and pHC.LeftText, visible)
    SetSafeShown(pHC and pHC.HealthBarText, visible)
    SetSafeShown(pHC and pHC.RightText, visible)
    SetSafeShown(PlayerFrameManaBarText, visible)
    if pPower then
      SetSafeShown(pPower.LeftText, visible)
      SetSafeShown(pPower.TextString, visible)
      SetSafeShown(pPower.RightText, visible)
    end
    local tMain = TargetFrame and TargetFrame.TargetFrameContent and TargetFrame.TargetFrameContent.TargetFrameContentMain
    local tHC = tMain and tMain.HealthBarsContainer
    local tPower = tMain and tMain.ManaBar
    ApplyBarTextSuppression(tHC and tHC.HealthBar, not visible)
    ApplyBarTextSuppression(tPower, not visible)
    SetSafeShown(tHC and tHC.LeftText, visible)
    SetSafeShown(tHC and tHC.HealthBarText, visible)
    SetSafeShown(tHC and tHC.RightText, visible)
    if tPower then
      SetSafeShown(tPower.LeftText, visible)
      SetSafeShown(tPower.TextString, visible)
      SetSafeShown(tPower.RightText, visible)
    end
    local fMain = FocusFrame and FocusFrame.TargetFrameContent and FocusFrame.TargetFrameContent.TargetFrameContentMain
    local fHC = fMain and fMain.HealthBarsContainer
    local fPower = fMain and fMain.ManaBar
    ApplyBarTextSuppression(fHC and fHC.HealthBar, not visible)
    ApplyBarTextSuppression(fPower, not visible)
    SetSafeShown(fHC and fHC.LeftText, visible)
    SetSafeShown(fHC and fHC.HealthBarText, visible)
    SetSafeShown(fHC and fHC.RightText, visible)
    if fPower then
      SetSafeShown(fPower.LeftText, visible)
      SetSafeShown(fPower.TextString, visible)
      SetSafeShown(fPower.RightText, visible)
    end
  end
  addonTable.ApplyUnitFrameStatusTexts = function()
    local p = addonTable.GetProfile and addonTable.GetProfile()
    if not p then return end
    if p.enableUnitFrameCustomization == false then
      SetDefaultStatusTextsVisible(true)
      if State.ufCustomTextTriplets then
        for _, t in pairs(State.ufCustomTextTriplets) do
          SetSafeShown(t.left, false)
          SetSafeShown(t.mid, false)
          SetSafeShown(t.right, false)
        end
      end
      return
    end
    SetDefaultStatusTextsVisible(false)
    local healthMode = NormalizeUFTextMode(p.ufHealthTextMode)
    local powerMode = NormalizeUFTextMode(p.ufPowerTextMode)
    local pMain = PlayerFrame and PlayerFrame.PlayerFrameContent and PlayerFrame.PlayerFrameContent.PlayerFrameContentMain
    local pHC = pMain and pMain.HealthBarsContainer
    local pPower = PlayerFrameManaBar or (pMain and pMain.ManaBarArea and pMain.ManaBarArea.ManaBar)
    local pHealthT = EnsureUFCustomTextTriplet("playerHealth", pHC and pHC.HealthBar)
    local pPowerT = EnsureUFCustomTextTriplet("playerPower", pPower)
    if pHealthT then
      SetSafeShown(pHealthT.left, true); SetSafeShown(pHealthT.mid, true); SetSafeShown(pHealthT.right, true)
      ApplyUFTextMode(pHealthT.left, pHealthT.mid, pHealthT.right, healthMode, UnitHealth("player"), UnitHealthMax("player"))
    end
    if pPowerT then
      SetSafeShown(pPowerT.left, true); SetSafeShown(pPowerT.mid, true); SetSafeShown(pPowerT.right, true)
      ApplyUFTextMode(pPowerT.left, pPowerT.mid, pPowerT.right, powerMode, UnitPower("player"), UnitPowerMax("player"))
    end
    local tMain = TargetFrame and TargetFrame.TargetFrameContent and TargetFrame.TargetFrameContent.TargetFrameContentMain
    local tHC = tMain and tMain.HealthBarsContainer
    local tPower = tMain and tMain.ManaBar
    local tHealthT = EnsureUFCustomTextTriplet("targetHealth", tHC and tHC.HealthBar)
    local tPowerT = EnsureUFCustomTextTriplet("targetPower", tPower)
    if tHealthT then
      SetSafeShown(tHealthT.left, true); SetSafeShown(tHealthT.mid, true); SetSafeShown(tHealthT.right, true)
      ApplyUFTextMode(tHealthT.left, tHealthT.mid, tHealthT.right, healthMode, UnitHealth("target"), UnitHealthMax("target"))
    end
    if tPowerT then
      SetSafeShown(tPowerT.left, true); SetSafeShown(tPowerT.mid, true); SetSafeShown(tPowerT.right, true)
      ApplyUFTextMode(tPowerT.left, tPowerT.mid, tPowerT.right, powerMode, UnitPower("target"), UnitPowerMax("target"))
    end
    local fMain = FocusFrame and FocusFrame.TargetFrameContent and FocusFrame.TargetFrameContent.TargetFrameContentMain
    local fHC = fMain and fMain.HealthBarsContainer
    local fPower = fMain and fMain.ManaBar
    local fHealthT = EnsureUFCustomTextTriplet("focusHealth", fHC and fHC.HealthBar)
    local fPowerT = EnsureUFCustomTextTriplet("focusPower", fPower)
    if fHealthT then
      SetSafeShown(fHealthT.left, true); SetSafeShown(fHealthT.mid, true); SetSafeShown(fHealthT.right, true)
      ApplyUFTextMode(fHealthT.left, fHealthT.mid, fHealthT.right, healthMode, UnitHealth("focus"), UnitHealthMax("focus"))
    end
    if fPowerT then
      SetSafeShown(fPowerT.left, true); SetSafeShown(fPowerT.mid, true); SetSafeShown(fPowerT.right, true)
      ApplyUFTextMode(fPowerT.left, fPowerT.mid, fPowerT.right, powerMode, UnitPower("focus"), UnitPowerMax("focus"))
    end
  end
  if addonTable.ApplyUnitFrameStatusTexts then addonTable.ApplyUnitFrameStatusTexts() end
  local function ApplyBorderTextureColor(frameObj, r, g, b)
    local tex = frameObj and frameObj.TargetFrameContainer and frameObj.TargetFrameContainer.FrameTexture
    if frameObj == PlayerFrame then
      tex = frameObj and frameObj.PlayerFrameContainer and frameObj.PlayerFrameContainer.FrameTexture
    end
    if tex and tex.SetVertexColor then
      tex:SetVertexColor(r or 1, g or 1, b or 1, 1)
    end
  end
  local borderR = profile.ufCustomBorderColorR or profile.ufBorderPlayerColorR or 1
  local borderG = profile.ufCustomBorderColorG or profile.ufBorderPlayerColorG or 1
  local borderB = profile.ufCustomBorderColorB or profile.ufBorderPlayerColorB or 1
  ...
  local function SetUFBaseHealthBarCovered(o, hp, covered, container, rootFrame)
    ...
    if not o.baseHpVisualStates then
      ...
      local tempLoss = rootFrame and rootFrame.PlayerFrameTempMaxHealthLoss
      if tempLoss then
        o.tempLossState = {
    ...
    if covered then
      ...
      if statusTex and statusTex.Hide then statusTex:Hide() end
  ...
  local function ApplyUFBigHBForFrame(key, rootFrame, hp, mp, portrait, isPlayerFrame)
    local enabled = profile.ufBigHBEnabled == true
    local frameTex
    if key == "player" then
    ...
    elseif key == "focus" then
      ...
      if bossIndex then
        ...
        frameTex = bossFrame and bossFrame.TargetFrameContainer and bossFrame.TargetFrameContainer.FrameTexture
      end
    end
    ...
    elseif string.find(key, "boss", 1, true) == 1 then
      enabled = enabled and profile.ufBigHBBossEnabled == true
    else
      enabled = false
    end
    local contextual = (key == "player" and rootFrame and rootFrame.Contextual) or nil
    if not enabled or not rootFrame or not hp or not hp.GetWidth then
      if hp and State.ufBigHBOverlays and State.ufBigHBOverlays[key] then
        SetUFBaseHealthBarCovered(State.ufBigHBOverlays[key], hp, false, rootFrame and rootFrame.HealthBarsContainer, rootFrame)
      end
      if key == "player" then
    ...
    if key == "player" then
      ...
      parent = (portrait and portrait.GetParent and portrait:GetParent()) or rootFrame
    end
    if not parent then
      parent = (rootFrame and rootFrame.HealthBarsContainer) or rootFrame
    end
    ...
    local hX, hY, hW, hH, bX, bY, bW, bH, bBright
    if isPlayerFrame then
      hX = 0
      hY = 8
      hW = -1
      ...
      hX = profile.ufBigHBOtherHealthX or 0
      hY = profile.ufBigHBOtherHealthY or 0
      hW = profile.ufBigHBOtherHealthW or 0
      hH = profile.ufBigHBOtherHealthH or 0
      bX = profile.ufBigHBOtherBgX or 0
      bY = profile.ufBigHBOtherBgY or 0
      bW = profile.ufBigHBOtherBgW or 0
      bH = profile.ufBigHBOtherBgH or 0
      bBright = profile.ufBigHBOtherBgBrightness or 1
    end
    if bBright < 0 then bBright = 0 end
    if bBright > 2 then bBright = 2 end
    local hpW = hp:GetWidth() or 0
    local hpH = hp:GetHeight() or 0
    if hpW < 1 then hpW = 1 end
    if hpH < 1 then hpH = 1 end
    local anchorPower = (mp and mp.GetWidth) and mp or hp
    local mpW = anchorPower:GetWidth() or 0
    local mpH = anchorPower:GetHeight() or 0
    if mpW < 1 then mpW = hpW end
    if mpH < 1 then mpH = hpH end
    local healthW = hpW + hW
    local healthH = hpH + hH
    if healthW < 1 then healthW = 1 end
    if healthH < 1 then healthH = 1 end
    local bgW = mpW + bW
    local bgH = mpH + bH
    if bgW < 1 then bgW = 1 end
    ...
    local portraitLevel = (portrait and portrait.GetFrameLevel and portrait:GetFrameLevel()) or nil
    ...
    if frameTex then
      ...
      if key == "player" and frameTex.SetFrameStrata then
      ...
      if key == "player" then
        ...
        if frameTex.SetAlpha then frameTex:SetAlpha(0) end
        if frameTex.Hide then frameTex:Hide() end
      end
    end
    if key == "player" then
    ...
    o.healthFrame:SetFrameLevel(math.max(bgLevel, ovLevel))
    o.bgFrame:ClearAllPoints()
    o.bgFrame:SetPoint("CENTER", anchorPower, "CENTER", bX, bY)
    o.bgFrame:SetSize(bgW, bgH)
    o.healthFrame:ClearAllPoints()
    o.healthFrame:SetPoint("CENTER", hp, "CENTER", hX, hY)
    o.healthFrame:SetSize(healthW, healthH)
    ...
    SetUFBaseHealthBarCovered(o, hp, true, rootFrame and rootFrame.HealthBarsContainer, rootFrame)
    o.bgFrame:Show()
    o.healthFrame:Show()
  end
  do
    ...
    local targetRoot = TargetFrame and TargetFrame.TargetFrameContent and TargetFrame.TargetFrameContent.TargetFrameContentMain
    local targetHC = targetRoot and targetRoot.HealthBarsContainer
    local targetHB = targetHC and targetHC.HealthBar
    local targetMP = targetRoot and targetRoot.ManaBar
    local targetPortrait = TargetFrame and TargetFrame.TargetFrameContainer
    ApplyUFBigHBForFrame("target", targetRoot or TargetFrame, targetHB, targetMP, targetPortrait, false)
    local focusRoot = FocusFrame and FocusFrame.TargetFrameContent and FocusFrame.TargetFrameContent.TargetFrameContentMain
    local focusHC = focusRoot and focusRoot.HealthBarsContainer
    local focusHB = focusHC and focusHC.HealthBar
    local focusMP = focusRoot and focusRoot.ManaBar
    local focusPortrait = FocusFrame and FocusFrame.TargetFrameContainer
    ApplyUFBigHBForFrame("focus", focusRoot or FocusFrame, focusHB, focusMP, focusPortrait, false)
    for i = 1, 5 do
      local bossFrame = _G["Boss" .. i .. "TargetFrame"]
      local bossRoot = bossFrame and bossFrame.TargetFrameContent and bossFrame.TargetFrameContent.TargetFrameContentMain
      local bossHC = bossRoot and bossRoot.HealthBarsContainer
      local bossHB = bossHC and bossHC.HealthBar
      local bossMP = bossRoot and bossRoot.ManaBar
      local bossPortrait = bossFrame and bossFrame.TargetFrameContainer
      ApplyUFBigHBForFrame("boss" .. i, bossRoot or bossFrame, bossHB, bossMP, bossPortrait, false)
    end
  end
  ...
  local selectedTexture = profile.useBiggerPlayerHealthframeTexture or "solid"
  ...
  local function ApplyBiggerHealthForUnit(frame, enabled, stateKey, unitToken)
    ...
    ApplyUnitHealthColor(hp, unitToken, profile.useBiggerPlayerHealthframeClassColor == true)
    if enabled then
  ...
  local function ApplyPlayerHealthColor()
    ...
    local useClassColor = profile.useBiggerPlayerHealthframeClassColor == true
    if profile.useBiggerPlayerHealthframe then
  ...
  if not orig.unitHealthColorHooked and type(hooksecurefunc) == "function" and type(UnitFrameHealthBar_Update) == "function" then
    ...
    hooksecurefunc("UnitFrameHealthBar_Update", function(statusBar, unitToken)
      ...
      if InCombatLockdown and InCombatLockdown() then return end
      local useClassColor = p.useBiggerPlayerHealthframeClassColor == true
      local ufBigEnabled = p.ufBigHBEnabled == true
      if not useClassColor and not p.useBiggerPlayerHealthframe and not ufBigEnabled then return end
      local pMain = PlayerFrame and PlayerFrame.PlayerFrameContent and PlayerFrame.PlayerFrameContent.PlayerFrameContentMain
      local tMain = TargetFrame and TargetFrame.TargetFrameContent and TargetFrame.TargetFrameContent.TargetFrameContentMain
      local fMain = FocusFrame and FocusFrame.TargetFrameContent and FocusFrame.TargetFrameContent.TargetFrameContentMain
      local targetBar = tMain and tMain.HealthBarsContainer and tMain.HealthBarsContainer.HealthBar
      ...
      if ufBigEnabled and State.ufBigHBOverlays then
        local function RefreshUFBigOverlay(key, root, hpBar)
          local ov = State.ufBigHBOverlays[key]
          if not ov or not hpBar then return end
          ...
          if frameTex and key ~= "player" then
            if frameTex.SetAlpha then frameTex:SetAlpha(0) end
            if frameTex.Hide then frameTex:Hide() end
          elseif frameTex and key == "player" then
            local contextual = pMain and pMain.Contextual
            if ov and ov.bgFrame and frameTex.SetFrameLevel then
              local maxLevel = 1000
              if ov.healthFrame and ov.healthFrame.GetFrameLevel then
                local hl = ov.healthFrame:GetFrameLevel() or maxLevel
                maxLevel = math.min(maxLevel, hl - 1)
              end
              local targetLevel = math.max(1, maxLevel - 2)
              frameTex:SetFrameLevel(targetLevel)
            end
            ...
            if ov and ov.frameTexAlpha ~= nil and frameTex.SetAlpha then frameTex:SetAlpha(ov.frameTexAlpha) end
            if frameTex.Show then frameTex:Show() end
            if ov and ov.frameTexHost and ov.frameTexParent then
              if ov.frameTexHost.SetFrameStrata then ov.frameTexHost:SetFrameStrata("BACKGROUND") end
              if ov.frameTexHost.SetFrameLevel then ov.frameTexHost:SetFrameLevel(0) end
              if ov.frameTexHost.SetParent then ov.frameTexHost:SetParent(ov.frameTexParent) end
              if ov.frameTexHost.ClearAllPoints and ov.frameTexHost.SetPoint then
                ov.frameTexHost:ClearAllPoints()
                ov.frameTexHost:SetPoint("TOPLEFT", ov.frameTexParent, "TOPLEFT", 0, 0)
                ov.frameTexHost:SetPoint("BOTTOMRIGHT", ov.frameTexParent, "BOTTOMRIGHT", 0, 0)
              end
              if frameTex.SetParent and frameTex.GetParent and frameTex:GetParent() ~= ov.frameTexHost then
                frameTex:SetParent(ov.frameTexHost)
              end
              if frameTex.ClearAllPoints and frameTex.SetPoint then
                frameTex:ClearAllPoints()
                local restored = false
                if ov.frameTexPoints and #ov.frameTexPoints > 0 then
                  for i = 1, #ov.frameTexPoints do
                    local pt = ov.frameTexPoints[i]
                    if pt and type(pt[1]) == "string" then
                      pcall(frameTex.SetPoint, frameTex, pt[1], pt[2], pt[3], pt[4], pt[5])
                      restored = true
                    end
                ...
                if (not restored) and frameTex.SetAllPoints then
                  frameTex:SetAllPoints(ov.frameTexHost)
                end
              end
              if ov.frameTexHost.Show then ov.frameTexHost:Show() end
            end
            if contextual then
              if contextual.SetFrameStrata then contextual:SetFrameStrata("BACKGROUND") end
              if contextual.SetFrameLevel then contextual:SetFrameLevel(0) end
        ...
        RefreshUFBigOverlay("player", pMain, healthBar)
        RefreshUFBigOverlay("target", tMain, targetBar)
        RefreshUFBigOverlay("focus", fMain, focusBar)
        for i = 1, 5 do
          local bf = _G["Boss" .. i .. "TargetFrame"]
          local bMain = bf and bf.TargetFrameContent and bf.TargetFrameContent.TargetFrameContentMain
          local bBar = bMain and bMain.HealthBarsContainer and bMain.HealthBarsContainer.HealthBar
          RefreshUFBigOverlay("boss" .. i, bMain, bBar)
        end
      end
    end)
  end
  if not orig.powerHooked and powerBar.HookScript then
    ...
    powerBar:HookScript("OnShow", function(bar)
      local p = addonTable.GetProfile and addonTable.GetProfile()
      if p and p.useBiggerPlayerHealthframe then
        bar:SetAlpha(0)
      end
    end)
  end
  ...
  if not orig.ufStatusTextHooked and type(hooksecurefunc) == "function" then
    ...
    if type(UnitFrameManaBar_Update) == "function" then
      hooksecurefunc("UnitFrameManaBar_Update", function()
        if addonTable.ApplyUnitFrameStatusTexts then addonTable.ApplyUnitFrameStatusTexts() end
      end)
    elseif type(UnitFramePowerBar_Update) == "function" then
      hooksecurefunc("UnitFramePowerBar_Update", function()
        if addonTable.ApplyUnitFrameStatusTexts then addonTable.ApplyUnitFrameStatusTexts() end
      end)
    end
    orig.ufStatusTextHooked = true
  end
  if not orig.ufStatusTextEventFrame then
    ...
    ev:SetScript("OnEvent", function(_, _, unit)
      if unit and unit ~= "player" and unit ~= "target" and unit ~= "focus" then return end
      if C_Timer and C_Timer.After then
        C_Timer.After(0, function()
          if addonTable.ApplyUnitFrameStatusTexts then addonTable.ApplyUnitFrameStatusTexts() end
        end)
      elseif addonTable.ApplyUnitFrameStatusTexts then
        addonTable.ApplyUnitFrameStatusTexts()
      end
    end)
    orig.ufStatusTextEventFrame = ev
  end
  ...
  local function CollectPlayerGlowTargets()
    ...
    addRestedRegions(pf)
    local hb = hContainer and hContainer.HealthBar
    if hb then
      add(hb.OverAbsorbGlow)
      add(hb.OverHealAbsorbGlow)
      add(hb.HealAbsorbBar)
      add(hb.TotalAbsorbBar)
      add(hb.MyHealPredictionBar)
      add(hb.OtherHealPredictionBar)
      add(hb.AnimatedLossBar)
    end
    return targets
  end
  local function IsGlowSuppressionEnabled()
    ...
    return p.useBiggerPlayerHealthframeDisableGlows == true
  end
  ...
  if profile.useBiggerPlayerHealthframe == true then
  ...
  local disableGlows = profile.useBiggerPlayerHealthframeDisableGlows == true
  if disableGlows then
    ApplyGlowSuppression(true)
  else
    ApplyGlowSuppression(false)
  end
  local disableCombatText = profile.useBiggerPlayerHealthframeDisableCombatText == true
...
local function UpdatePRB()
  ...
  local borderSize = profile.prbBorderSize or 1
          ...
          for i = 1, maxClassPower do
            ...
            if seg then
              ...
              if borderSize > 0 then
                ...
                seg.border:SetBackdrop({edgeFile = "Interface\\Buttons\\WHITE8x8", edgeSize = borderSize})
                seg.border:SetBackdropBorderColor(0, 0, 0, 1)
                seg.border:Show()
              else
...
local function UpdateCastbar()
  ...
  castbarFrame.bar:SetValue(progress)
  if showSpellName then
    -- name can be a secret value in Midnight; avoid Lua-side comparisons.
...
addonTable.UpdateFocusCastbar = function()
  ...
  if showSpellName then
    -- name can be a secret value in Midnight; never compare/cache it in Lua.
...
local function GetProfile()
  ...
  if type(profile.ufHealthTextMode) ~= "string" then
    profile.ufHealthTextMode = defaults.profiles.Default.ufHealthTextMode
  end
  if type(profile.ufPowerTextMode) ~= "string" then
    profile.ufPowerTextMode = defaults.profiles.Default.ufPowerTextMode
  end
  if profile.ufBigHBEnabled == nil then profile.ufBigHBEnabled = defaults.profiles.Default.ufBigHBEnabled end
  if profile.ufBigHBPlayerEnabled == nil then profile.ufBigHBPlayerEnabled = defaults.profiles.Default.ufBigHBPlayerEnabled end
  if profile.ufBigHBTargetEnabled == nil then profile.ufBigHBTargetEnabled = defaults.profiles.Default.ufBigHBTargetEnabled end
  if profile.ufBigHBFocusEnabled == nil then profile.ufBigHBFocusEnabled = defaults.profiles.Default.ufBigHBFocusEnabled end
  if profile.ufBigHBBossEnabled == nil then profile.ufBigHBBossEnabled = defaults.profiles.Default.ufBigHBBossEnabled end
  if profile.ufBigHBPlayerHealthX == nil then profile.ufBigHBPlayerHealthX = defaults.profiles.Default.ufBigHBPlayerHealthX end
  if profile.ufBigHBPlayerHealthY == nil then profile.ufBigHBPlayerHealthY = defaults.profiles.Default.ufBigHBPlayerHealthY end
  if profile.ufBigHBPlayerHealthW == nil then profile.ufBigHBPlayerHealthW = defaults.profiles.Default.ufBigHBPlayerHealthW end
  if profile.ufBigHBPlayerHealthH == nil then profile.ufBigHBPlayerHealthH = defaults.profiles.Default.ufBigHBPlayerHealthH end
  if profile.ufBigHBPlayerBgX == nil then profile.ufBigHBPlayerBgX = defaults.profiles.Default.ufBigHBPlayerBgX end
  if profile.ufBigHBPlayerBgY == nil then profile.ufBigHBPlayerBgY = defaults.profiles.Default.ufBigHBPlayerBgY end
  if profile.ufBigHBPlayerBgW == nil then profile.ufBigHBPlayerBgW = defaults.profiles.Default.ufBigHBPlayerBgW end
  if profile.ufBigHBPlayerBgH == nil then profile.ufBigHBPlayerBgH = defaults.profiles.Default.ufBigHBPlayerBgH end
  if profile.ufBigHBPlayerBgBrightness == nil then profile.ufBigHBPlayerBgBrightness = defaults.profiles.Default.ufBigHBPlayerBgBrightness end
  if profile.ufBigHBOtherHealthX == nil then profile.ufBigHBOtherHealthX = defaults.profiles.Default.ufBigHBOtherHealthX end
  if profile.ufBigHBOtherHealthY == nil then profile.ufBigHBOtherHealthY = defaults.profiles.Default.ufBigHBOtherHealthY end
  if profile.ufBigHBOtherHealthW == nil then profile.ufBigHBOtherHealthW = defaults.profiles.Default.ufBigHBOtherHealthW end
  if profile.ufBigHBOtherHealthH == nil then profile.ufBigHBOtherHealthH = defaults.profiles.Default.ufBigHBOtherHealthH end
  if profile.ufBigHBOtherBgX == nil then profile.ufBigHBOtherBgX = defaults.profiles.Default.ufBigHBOtherBgX end
  if profile.ufBigHBOtherBgY == nil then profile.ufBigHBOtherBgY = defaults.profiles.Default.ufBigHBOtherBgY end
  if profile.ufBigHBOtherBgW == nil then profile.ufBigHBOtherBgW = defaults.profiles.Default.ufBigHBOtherBgW end
  if profile.ufBigHBOtherBgH == nil then profile.ufBigHBOtherBgH = defaults.profiles.Default.ufBigHBOtherBgH end
  if profile.ufBigHBOtherBgBrightness == nil then profile.ufBigHBOtherBgBrightness = defaults.profiles.Default.ufBigHBOtherBgBrightness end
  if profile.ufBigHBEnabled == true then
    profile.useBiggerPlayerHealthframe = false
    profile.useBiggerPlayerHealthframeClassColor = false
  end
...
addonTable.CursorTrackingOnUpdate = function(self, elapsed)
  ...
  local x, y = GetCursorPosition()
  local lastTrackX = State.lastTrackCursorX or 0
  local lastTrackY = State.lastTrackCursorY or 0
  if math.abs(x - lastTrackX) <= State.cursorMoveThreshold and math.abs(y - lastTrackY) <= State.cursorMoveThreshold then
    return
  end
