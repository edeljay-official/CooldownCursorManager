...
local function UpdateAllControls()
  ...
  if addonTable.bigHealthframePlayerCB then addonTable.bigHealthframePlayerCB:SetChecked(profile.useBiggerPlayerHealthframe == true) end
  local ufBigEnabled = profile.ufBigHBEnabled == true
  if addonTable.bigHealthframePlayerClassColorCB then
    if ufBigEnabled and profile.useBiggerPlayerHealthframeClassColor == true then
      profile.useBiggerPlayerHealthframeClassColor = false
    end
    local classColorEnabled = not ufBigEnabled
    addonTable.bigHealthframePlayerClassColorCB:SetChecked(classColorEnabled and profile.useBiggerPlayerHealthframeClassColor == true)
    addonTable.bigHealthframePlayerClassColorCB:SetEnabled(classColorEnabled)
    addonTable.bigHealthframePlayerClassColorCB:SetAlpha(classColorEnabled and 1 or 0.5)
  end
  if addonTable.bigHealthframePlayerDisableGlowsCB then
    addonTable.bigHealthframePlayerDisableGlowsCB:SetChecked(profile.useBiggerPlayerHealthframeDisableGlows == true)
    local enableGlows = true
    addonTable.bigHealthframePlayerDisableGlowsCB:SetEnabled(enableGlows)
    if addonTable.bigHealthframePlayerDisableGlowsCB.SetAlpha then
      addonTable.bigHealthframePlayerDisableGlowsCB:SetAlpha(enableGlows and 1 or 0.5)
    end
  end
  if addonTable.bigHealthframePlayerDisableCombatTextCB then
    addonTable.bigHealthframePlayerDisableCombatTextCB:SetChecked(profile.useBiggerPlayerHealthframeDisableCombatText == true)
    local enableCT = true
    addonTable.bigHealthframePlayerDisableCombatTextCB:SetEnabled(enableCT)
    if addonTable.bigHealthframePlayerDisableCombatTextCB.SetAlpha then
      addonTable.bigHealthframePlayerDisableCombatTextCB:SetAlpha(enableCT and 1 or 0.5)
    end
  end
  if addonTable.bigHealthframePlayerTextureDD then
    addonTable.bigHealthframePlayerTextureDD:SetValue(profile.useBiggerPlayerHealthframeTexture or "solid")
    local textureDDEnabled = not ufBigEnabled
    addonTable.bigHealthframePlayerTextureDD:SetEnabled(textureDDEnabled)
    if addonTable.bigHealthframePlayerTextureDD.SetAlpha then
      addonTable.bigHealthframePlayerTextureDD:SetAlpha(textureDDEnabled and 1 or 0.5)
    end
    if addonTable.bigHealthframePlayerTextureLbl then
      if textureDDEnabled then
        addonTable.bigHealthframePlayerTextureLbl:SetTextColor(0.9, 0.9, 0.9)
      else
        addonTable.bigHealthframePlayerTextureLbl:SetTextColor(0.5, 0.5, 0.5)
      end
    end
  end
  if addonTable.ufHealthTextDD then
    local mode = type(profile.ufHealthTextMode) == "string" and string.upper(profile.ufHealthTextMode) or "BOTH"
    if mode ~= "NUMERIC" and mode ~= "PERCENT" and mode ~= "BOTH" and mode ~= "NONE" then mode = "BOTH" end
    addonTable.ufHealthTextDD:SetValue(mode)
  end
  if addonTable.ufPowerTextDD then
    local mode = type(profile.ufPowerTextMode) == "string" and string.upper(profile.ufPowerTextMode) or "BOTH"
    if mode ~= "NUMERIC" and mode ~= "PERCENT" and mode ~= "BOTH" and mode ~= "NONE" then mode = "BOTH" end
    addonTable.ufPowerTextDD:SetValue(mode)
  end
  if addonTable.ufBorderColorSwatch then
    addonTable.ufBorderColorSwatch:SetBackdropColor(num(profile.ufCustomBorderColorR, 1), num(profile.ufCustomBorderColorG, 1), num(profile.ufCustomBorderColorB, 1), 1)
  end
  ...
  local ufBigBoss = profile.ufBigHBBossEnabled == true
  local ufBigOthers = ufBigTarget or ufBigFocus or ufBigBoss
  if addonTable.ufBigHBEnableCB then addonTable.ufBigHBEnableCB:SetChecked(ufBigEnabled) end
  if addonTable.ufBigHBPlayerCB then addonTable.ufBigHBPlayerCB:SetChecked(ufBigPlayer); addonTable.ufBigHBPlayerCB:SetEnabled(ufBigEnabled) end
  if addonTable.ufBigHBTargetCB then addonTable.ufBigHBTargetCB:SetChecked(ufBigTarget); addonTable.ufBigHBTargetCB:SetEnabled(ufBigEnabled) end
  if addonTable.ufBigHBFocusCB then addonTable.ufBigHBFocusCB:SetChecked(ufBigFocus); addonTable.ufBigHBFocusCB:SetEnabled(ufBigEnabled) end
  if addonTable.ufBigHBBossCB then addonTable.ufBigHBBossCB:SetChecked(ufBigBoss); addonTable.ufBigHBBossCB:SetEnabled(ufBigEnabled) end
  local function ApplyUFBigSlider(slider, value, enabled, fmt)
    if not slider then return end
    slider:SetValue(value)
    if slider.valueText then
      if fmt then
        slider.valueText:SetText(string.format(fmt, value))
      else
  ...
  local playerGroupEnabled = ufBigEnabled and ufBigPlayer
  local othersGroupEnabled = ufBigEnabled and ufBigOthers
  ApplyUFBigSlider(addonTable.ufBigHBPlayerHealthXSlider, num(profile.ufBigHBPlayerHealthX, 0), playerGroupEnabled)
  ApplyUFBigSlider(addonTable.ufBigHBPlayerHealthYSlider, num(profile.ufBigHBPlayerHealthY, 0), playerGroupEnabled)
  ApplyUFBigSlider(addonTable.ufBigHBPlayerHealthWSlider, num(profile.ufBigHBPlayerHealthW, 0), playerGroupEnabled)
  ApplyUFBigSlider(addonTable.ufBigHBPlayerHealthHSlider, num(profile.ufBigHBPlayerHealthH, 0), playerGroupEnabled)
  ApplyUFBigSlider(addonTable.ufBigHBPlayerBgXSlider, num(profile.ufBigHBPlayerBgX, 0), playerGroupEnabled)
  ApplyUFBigSlider(addonTable.ufBigHBPlayerBgYSlider, num(profile.ufBigHBPlayerBgY, 0), playerGroupEnabled)
  ApplyUFBigSlider(addonTable.ufBigHBPlayerBgWSlider, num(profile.ufBigHBPlayerBgW, 0), playerGroupEnabled)
  ApplyUFBigSlider(addonTable.ufBigHBPlayerBgHSlider, num(profile.ufBigHBPlayerBgH, 0), playerGroupEnabled)
  ApplyUFBigSlider(addonTable.ufBigHBPlayerBgBrightnessSlider, num(profile.ufBigHBPlayerBgBrightness, 1), playerGroupEnabled, "%.2f")
  ApplyUFBigSlider(addonTable.ufBigHBOtherHealthXSlider, num(profile.ufBigHBOtherHealthX, 0), othersGroupEnabled)
  ApplyUFBigSlider(addonTable.ufBigHBOtherHealthYSlider, num(profile.ufBigHBOtherHealthY, 0), othersGroupEnabled)
  ApplyUFBigSlider(addonTable.ufBigHBOtherHealthWSlider, num(profile.ufBigHBOtherHealthW, 0), othersGroupEnabled)
  ApplyUFBigSlider(addonTable.ufBigHBOtherHealthHSlider, num(profile.ufBigHBOtherHealthH, 0), othersGroupEnabled)
  ApplyUFBigSlider(addonTable.ufBigHBOtherBgXSlider, num(profile.ufBigHBOtherBgX, 0), othersGroupEnabled)
  ApplyUFBigSlider(addonTable.ufBigHBOtherBgYSlider, num(profile.ufBigHBOtherBgY, 0), othersGroupEnabled)
  ApplyUFBigSlider(addonTable.ufBigHBOtherBgWSlider, num(profile.ufBigHBOtherBgW, 0), othersGroupEnabled)
  ApplyUFBigSlider(addonTable.ufBigHBOtherBgHSlider, num(profile.ufBigHBOtherBgH, 0), othersGroupEnabled)
  ApplyUFBigSlider(addonTable.ufBigHBOtherBgBrightnessSlider, num(profile.ufBigHBOtherBgBrightness, 1), othersGroupEnabled, "%.2f")
...
local function InitHandlers()
  ...
  if addonTable.bigHealthframePlayerCB then
    addonTable.bigHealthframePlayerCB.customOnClick = function(s)
      local p = GetProfile()
      if not p then return end
      local enabled = s:GetChecked()
      p.useBiggerPlayerHealthframe = enabled
      if addonTable.ApplyUnitFrameCustomization then addonTable.ApplyUnitFrameCustomization() end
    end
  end
  if addonTable.bigHealthframePlayerClassColorCB then addonTable.bigHealthframePlayerClassColorCB.customOnClick = function(s) local p = GetProfile(); if p then p.useBiggerPlayerHealthframeClassColor = s:GetChecked(); if addonTable.ApplyUnitFrameCustomization then addonTable.ApplyUnitFrameCustomization() end end end end
  if addonTable.bigHealthframePlayerDisableGlowsCB then
    addonTable.bigHealthframePlayerDisableGlowsCB.customOnClick = function(s)
      local p = GetProfile(); if p then p.useBiggerPlayerHealthframeDisableGlows = s:GetChecked(); if addonTable.ApplyUnitFrameCustomization then addonTable.ApplyUnitFrameCustomization() end end
    end
  end
  if addonTable.bigHealthframePlayerDisableCombatTextCB then
    addonTable.bigHealthframePlayerDisableCombatTextCB.customOnClick = function(s)
      local p = GetProfile(); if p then p.useBiggerPlayerHealthframeDisableCombatText = s:GetChecked(); if addonTable.ApplyUnitFrameCustomization then addonTable.ApplyUnitFrameCustomization() end end
    end
  end
  if addonTable.bigHealthframePlayerTextureDD then addonTable.bigHealthframePlayerTextureDD.onSelect = function(v) local p = GetProfile(); if p then p.useBiggerPlayerHealthframeTexture = v; if addonTable.ApplyUnitFrameCustomization then addonTable.ApplyUnitFrameCustomization() end end end end
  local function ApplyUFBigHealthbarChanges()
    local p = GetProfile()
    if p and p.ufBigHBEnabled == true then
      p.useBiggerPlayerHealthframe = false
    end
    if addonTable.UpdateAllControls then addonTable.UpdateAllControls() end
    if addonTable.ApplyUnitFrameCustomization then addonTable.ApplyUnitFrameCustomization() end
  end
  if addonTable.ufBigHBEnableCB then addonTable.ufBigHBEnableCB.customOnClick = function(s) local p = GetProfile(); if p then p.ufBigHBEnabled = s:GetChecked(); if p.ufBigHBEnabled == true then p.useBiggerPlayerHealthframeClassColor = false end; ApplyUFBigHealthbarChanges() end end end
  if addonTable.ufBigHBPlayerCB then addonTable.ufBigHBPlayerCB.customOnClick = function(s) local p = GetProfile(); if p then p.ufBigHBPlayerEnabled = s:GetChecked(); ApplyUFBigHealthbarChanges() end end end
  if addonTable.ufBigHBTargetCB then addonTable.ufBigHBTargetCB.customOnClick = function(s) local p = GetProfile(); if p then p.ufBigHBTargetEnabled = s:GetChecked(); ApplyUFBigHealthbarChanges() end end end
  if addonTable.ufBigHBFocusCB then addonTable.ufBigHBFocusCB.customOnClick = function(s) local p = GetProfile(); if p then p.ufBigHBFocusEnabled = s:GetChecked(); ApplyUFBigHealthbarChanges() end end end
  if addonTable.ufBigHBBossCB then addonTable.ufBigHBBossCB.customOnClick = function(s) local p = GetProfile(); if p then p.ufBigHBBossEnabled = s:GetChecked(); ApplyUFBigHealthbarChanges() end end end
  if addonTable.ufBigHBPlayerHealthXSlider then addonTable.ufBigHBPlayerHealthXSlider:SetScript("OnValueChanged", function(s, v) local p = GetProfile(); if p then p.ufBigHBPlayerHealthX = math.floor(v + 0.5); s.valueText:SetText(math.floor(v + 0.5)); if addonTable.ApplyUnitFrameCustomization then addonTable.ApplyUnitFrameCustomization() end end end) end
  if addonTable.ufBigHBPlayerHealthYSlider then addonTable.ufBigHBPlayerHealthYSlider:SetScript("OnValueChanged", function(s, v) local p = GetProfile(); if p then p.ufBigHBPlayerHealthY = math.floor(v + 0.5); s.valueText:SetText(math.floor(v + 0.5)); if addonTable.ApplyUnitFrameCustomization then addonTable.ApplyUnitFrameCustomization() end end end) end
  if addonTable.ufBigHBPlayerHealthWSlider then addonTable.ufBigHBPlayerHealthWSlider:SetScript("OnValueChanged", function(s, v) local p = GetProfile(); if p then p.ufBigHBPlayerHealthW = math.floor(v + 0.5); s.valueText:SetText(math.floor(v + 0.5)); if addonTable.ApplyUnitFrameCustomization then addonTable.ApplyUnitFrameCustomization() end end end) end
  if addonTable.ufBigHBPlayerHealthHSlider then addonTable.ufBigHBPlayerHealthHSlider:SetScript("OnValueChanged", function(s, v) local p = GetProfile(); if p then p.ufBigHBPlayerHealthH = math.floor(v + 0.5); s.valueText:SetText(math.floor(v + 0.5)); if addonTable.ApplyUnitFrameCustomization then addonTable.ApplyUnitFrameCustomization() end end end) end
  if addonTable.ufBigHBPlayerBgXSlider then addonTable.ufBigHBPlayerBgXSlider:SetScript("OnValueChanged", function(s, v) local p = GetProfile(); if p then p.ufBigHBPlayerBgX = math.floor(v + 0.5); s.valueText:SetText(math.floor(v + 0.5)); if addonTable.ApplyUnitFrameCustomization then addonTable.ApplyUnitFrameCustomization() end end end) end
  if addonTable.ufBigHBPlayerBgYSlider then addonTable.ufBigHBPlayerBgYSlider:SetScript("OnValueChanged", function(s, v) local p = GetProfile(); if p then p.ufBigHBPlayerBgY = math.floor(v + 0.5); s.valueText:SetText(math.floor(v + 0.5)); if addonTable.ApplyUnitFrameCustomization then addonTable.ApplyUnitFrameCustomization() end end end) end
  if addonTable.ufBigHBPlayerBgWSlider then addonTable.ufBigHBPlayerBgWSlider:SetScript("OnValueChanged", function(s, v) local p = GetProfile(); if p then p.ufBigHBPlayerBgW = math.floor(v + 0.5); s.valueText:SetText(math.floor(v + 0.5)); if addonTable.ApplyUnitFrameCustomization then addonTable.ApplyUnitFrameCustomization() end end end) end
  if addonTable.ufBigHBPlayerBgHSlider then addonTable.ufBigHBPlayerBgHSlider:SetScript("OnValueChanged", function(s, v) local p = GetProfile(); if p then p.ufBigHBPlayerBgH = math.floor(v + 0.5); s.valueText:SetText(math.floor(v + 0.5)); if addonTable.ApplyUnitFrameCustomization then addonTable.ApplyUnitFrameCustomization() end end end) end
  if addonTable.ufBigHBPlayerBgBrightnessSlider then addonTable.ufBigHBPlayerBgBrightnessSlider:SetScript("OnValueChanged", function(s, v) local p = GetProfile(); if p then p.ufBigHBPlayerBgBrightness = v; s.valueText:SetText(string.format("%.2f", v)); if addonTable.ApplyUnitFrameCustomization then addonTable.ApplyUnitFrameCustomization() end end end) end
  if addonTable.ufBigHBOtherHealthXSlider then addonTable.ufBigHBOtherHealthXSlider:SetScript("OnValueChanged", function(s, v) local p = GetProfile(); if p then p.ufBigHBOtherHealthX = math.floor(v + 0.5); s.valueText:SetText(math.floor(v + 0.5)); if addonTable.ApplyUnitFrameCustomization then addonTable.ApplyUnitFrameCustomization() end end end) end
  if addonTable.ufBigHBOtherHealthYSlider then addonTable.ufBigHBOtherHealthYSlider:SetScript("OnValueChanged", function(s, v) local p = GetProfile(); if p then p.ufBigHBOtherHealthY = math.floor(v + 0.5); s.valueText:SetText(math.floor(v + 0.5)); if addonTable.ApplyUnitFrameCustomization then addonTable.ApplyUnitFrameCustomization() end end end) end
  if addonTable.ufBigHBOtherHealthWSlider then addonTable.ufBigHBOtherHealthWSlider:SetScript("OnValueChanged", function(s, v) local p = GetProfile(); if p then p.ufBigHBOtherHealthW = math.floor(v + 0.5); s.valueText:SetText(math.floor(v + 0.5)); if addonTable.ApplyUnitFrameCustomization then addonTable.ApplyUnitFrameCustomization() end end end) end
  if addonTable.ufBigHBOtherHealthHSlider then addonTable.ufBigHBOtherHealthHSlider:SetScript("OnValueChanged", function(s, v) local p = GetProfile(); if p then p.ufBigHBOtherHealthH = math.floor(v + 0.5); s.valueText:SetText(math.floor(v + 0.5)); if addonTable.ApplyUnitFrameCustomization then addonTable.ApplyUnitFrameCustomization() end end end) end
  if addonTable.ufBigHBOtherBgXSlider then addonTable.ufBigHBOtherBgXSlider:SetScript("OnValueChanged", function(s, v) local p = GetProfile(); if p then p.ufBigHBOtherBgX = math.floor(v + 0.5); s.valueText:SetText(math.floor(v + 0.5)); if addonTable.ApplyUnitFrameCustomization then addonTable.ApplyUnitFrameCustomization() end end end) end
  if addonTable.ufBigHBOtherBgYSlider then addonTable.ufBigHBOtherBgYSlider:SetScript("OnValueChanged", function(s, v) local p = GetProfile(); if p then p.ufBigHBOtherBgY = math.floor(v + 0.5); s.valueText:SetText(math.floor(v + 0.5)); if addonTable.ApplyUnitFrameCustomization then addonTable.ApplyUnitFrameCustomization() end end end) end
  if addonTable.ufBigHBOtherBgWSlider then addonTable.ufBigHBOtherBgWSlider:SetScript("OnValueChanged", function(s, v) local p = GetProfile(); if p then p.ufBigHBOtherBgW = math.floor(v + 0.5); s.valueText:SetText(math.floor(v + 0.5)); if addonTable.ApplyUnitFrameCustomization then addonTable.ApplyUnitFrameCustomization() end end end) end
  if addonTable.ufBigHBOtherBgHSlider then addonTable.ufBigHBOtherBgHSlider:SetScript("OnValueChanged", function(s, v) local p = GetProfile(); if p then p.ufBigHBOtherBgH = math.floor(v + 0.5); s.valueText:SetText(math.floor(v + 0.5)); if addonTable.ApplyUnitFrameCustomization then addonTable.ApplyUnitFrameCustomization() end end end) end
  if addonTable.ufBigHBOtherBgBrightnessSlider then addonTable.ufBigHBOtherBgBrightnessSlider:SetScript("OnValueChanged", function(s, v) local p = GetProfile(); if p then p.ufBigHBOtherBgBrightness = v; s.valueText:SetText(string.format("%.2f", v)); if addonTable.ApplyUnitFrameCustomization then addonTable.ApplyUnitFrameCustomization() end end end) end
  if addonTable.ufHealthTextDD then
    addonTable.ufHealthTextDD.onSelect = function(v)
      local p = GetProfile()
      if not p then return end
      local val = type(v) == "string" and string.upper(v) or "BOTH"
      if val ~= "NUMERIC" and val ~= "PERCENT" and val ~= "BOTH" and val ~= "NONE" then val = "BOTH" end
      p.ufHealthTextMode = val
      if addonTable.ApplyUnitFrameStatusTexts then addonTable.ApplyUnitFrameStatusTexts() end
      if addonTable.ApplyUnitFrameCustomization then addonTable.ApplyUnitFrameCustomization() end
    end
  end
  if addonTable.ufPowerTextDD then
    addonTable.ufPowerTextDD.onSelect = function(v)
      local p = GetProfile()
      if not p then return end
      local val = type(v) == "string" and string.upper(v) or "BOTH"
      if val ~= "NUMERIC" and val ~= "PERCENT" and val ~= "BOTH" and val ~= "NONE" then val = "BOTH" end
      p.ufPowerTextMode = val
      if addonTable.ApplyUnitFrameStatusTexts then addonTable.ApplyUnitFrameStatusTexts() end
      if addonTable.ApplyUnitFrameCustomization then addonTable.ApplyUnitFrameCustomization() end
    end
  end
  ...
  local exportCategoryKeys = {
    general = {"iconBorderSize", "iconStrata", "enableMasque", "showMinimapButton", "minimapButtonAngle",
               ...
               "enablePlayerDebuffs",
               "useBiggerPlayerHealthframe", "useBiggerPlayerHealthframeClassColor", "useBiggerPlayerHealthframeTexture",
               "useBiggerPlayerHealthframeDisableGlows", "useBiggerPlayerHealthframeDisableCombatText",
               "ufCustomBorderColorR", "ufCustomBorderColorG", "ufCustomBorderColorB",
               "ufHealthTextMode", "ufPowerTextMode",
               "ufBigHBEnabled", "ufBigHBPlayerEnabled", "ufBigHBTargetEnabled", "ufBigHBFocusEnabled", "ufBigHBBossEnabled",
               "ufBigHBPlayerHealthX", "ufBigHBPlayerHealthY", "ufBigHBPlayerHealthW", "ufBigHBPlayerHealthH",
               "ufBigHBPlayerBgX", "ufBigHBPlayerBgY", "ufBigHBPlayerBgW", "ufBigHBPlayerBgH", "ufBigHBPlayerBgBrightness",
               "ufBigHBOtherHealthX", "ufBigHBOtherHealthY", "ufBigHBOtherHealthW", "ufBigHBOtherHealthH",
               "ufBigHBOtherBgX", "ufBigHBOtherBgY", "ufBigHBOtherBgW", "ufBigHBOtherBgH", "ufBigHBOtherBgBrightness",
               ...
               "customBarX", "customBarY", "customBarCentered", "customBarCooldownMode", "customBarIconsPerRow",
               ...
               "customBar2X", "customBar2Y", "customBar2Centered", "customBar2CooldownMode", "customBar2IconsPerRow",
               ...
               "customBar3X", "customBar3Y", "customBar3Centered", "customBar3CooldownMode", "customBar3IconsPerRow",
    ...
    cursor = {"iconSize", "iconSpacing", "offsetX", "offsetY", "cdTextScale", "stackTextScale",
              ...
              "stackTextPosition", "stackTextOffsetX", "stackTextOffsetY", "cooldownIconMode",
