...
local function CCM_FormatBossHealthTextSafe(cur, unit, fmt)
  ...
  elseif fmt == "full" then
    if CCM_IsSecret(cur) then
      return CCM_SafeAbbrevValue(cur) or ""
    end
    local n = tonumber(cur)
    if n then return string.format("%d", n) end
    return ""
  else
    local pctStr = CCM_SafePercent(unit)
    local valStr = CCM_SafeAbbrevValue(cur)
    if pctStr and valStr and valStr ~= "" then return pctStr .. " (" .. valStr .. ")" end
    return pctStr or valStr or ""
  end
end
  ...
  st.holder:SetScript("OnUpdate", function(_, elapsed)
    ...
    for i = 1, #st.rows do
      ...
      if UnitExists(unit) then
        ...
        local castName, _, castIcon, _, _, _, _, castNI = UnitCastingInfo(unit)
        local isCasting = castName ~= nil
        local isChanneling = false
        if not isCasting then
          castName, _, castIcon, _, _, _, castNI = UnitChannelInfo(unit)
          isChanneling = castName ~= nil
        end
        if isCasting or isChanneling then
          showingCast = true
          local durationObject = nil
          if isCasting and UnitCastingDuration then
            local okDur, durObj = pcall(UnitCastingDuration, unit)
            if okDur then durationObject = durObj end
          elseif isChanneling and UnitChannelDuration then
          ...
          if durationObject and row.cast.SetTimerDuration then
            ...
            local dir = isCasting and Enum.StatusBarTimerDirection.ElapsedTime or Enum.StatusBarTimerDirection.RemainingTime
            pcall(row.cast.SetTimerDuration, row.cast, durationObject, interp, dir)
          end
          local ni = false
          pcall(function() if castNI then ni = true end end)
          if ni then
            row.cast:SetStatusBarColor(0.50, 0.50, 0.50, 1)
          else
            row.cast:SetStatusBarColor(isCasting and 1.00 or 0.34, isCasting and 0.72 or 0.58, isCasting and 0.12 or 1.00, 1)
          end
          row.castText:SetText(castName or "")
          ...
          if row.castIcon and castIcon then row.castIcon:SetTexture(castIcon) end
        end
...
local function CCM_ApplyCustomBossFrames(profile, ufEnabled, useCustomTex, selectedTexturePath)
  ...
  local healthTextFormat = profile.ufBossHealthTextFormat or "percent_value"
...
addonTable.ApplyUnitFrameCustomization = function()
  ...
  local function ApplyBossFrameCustomization()
    ...
    local function UpdateCustomBossCastbar(st, unitToken)
      ...
      local showing = false
      local castOk, castName, castIcon, castNI = pcall(function()
        local cn, ci, _, _, _, _, _, ni = UnitCastingInfo(unitToken)
        local plainNI = false
        if ni then plainNI = true end
        return cn, ci, plainNI
      end)
      if castOk and castName then
        showing = true
        local okDur, durObj = pcall(UnitCastingDuration, unitToken)
        if okDur and durObj and bar.SetTimerDuration then
          local interp = Enum and Enum.StatusBarInterpolation and Enum.StatusBarInterpolation.Immediate or nil
          pcall(bar.SetTimerDuration, bar, durObj, interp, Enum.StatusBarTimerDirection.ElapsedTime)
          if st.castTime then
            local okRem, rem = pcall(function() return durObj:GetRemainingDuration() end)
            if okRem and rem ~= nil then st.castTime:SetText(string.format("%.1f", rem)) end
          end
        end
        bar:SetStatusBarColor(castNI and 0.50 or 1.00, castNI and 0.50 or 0.72, castNI and 0.50 or 0.12, 1)
        if st.castText then st.castText:SetText(castName or "") end
      else
        local chOk, chName, chNI = pcall(function()
          local cn, _, _, _, _, _, ni = UnitChannelInfo(unitToken)
          local plainNI = false
          if ni then plainNI = true end
          return cn, plainNI
        end)
        if chOk and chName then
          showing = true
          local okDur, durObj = pcall(UnitChannelDuration, unitToken)
          if okDur and durObj and bar.SetTimerDuration then
            local interp = Enum and Enum.StatusBarInterpolation and Enum.StatusBarInterpolation.Immediate or nil
            pcall(bar.SetTimerDuration, bar, durObj, interp, Enum.StatusBarTimerDirection.RemainingTime)
            if st.castTime then
              local okRem, rem = pcall(function() return durObj:GetRemainingDuration() end)
              if okRem and rem ~= nil then st.castTime:SetText(string.format("%.1f", rem)) end
            end
          end
          bar:SetStatusBarColor(chNI and 0.50 or 0.34, chNI and 0.50 or 0.58, chNI and 0.50 or 1.00, 1)
          if st.castText then st.castText:SetText(chName or "") end
        end
      end
    ...
    for idx = 1, #list do
      ...
      if enabled then
        ...
        if st.castHolder and st.castBar then
          ...
          st.castHolder:SetScript("OnUpdate", function(_, elapsed)
        ...
        if st.castHolder then
          st.castHolder:SetScript("OnUpdate", nil)
          st.castHolder:Hide()
        end
